...
import '../../features/ctf/forensic_challenge.dart' show ForensicChallenge;
import '../../features/ctf/web_challenge.dart';
import '../../features/ctf/reverse_challenge.dart';
import '../../features/report/report_page.dart';
import '../../features/report/report_history_page.dart';
...
final GoRouter appRouter = GoRouter(
  ...
  redirect: (BuildContext context, GoRouterState state) {
    ...
    // Onboarding check: if onboarding not completed and not on onboarding/auth routes, show onboarding
    // (This is a simplified check — the actual onboarding state is checked in LoginPage)

    // If not logged in, require login (unless already on auth or onboarding routes)
    if (!isLoggedIn && !isAuthRoute && !isOnboarding) return AppConstants.routeLogin;
  ...
  routes: [
      ...
      routes: [
        ...
        GoRoute(
          ...
          routes: [
            GoRoute(
              path: 'password',
              builder: (_, __) => const PasswordCheckerPage(),
            ),
            GoRoute(
              path: 'base64',
              builder: (_, __) => const Base64ToolPage(),
            ),
            GoRoute(
              path: 'hash',
              builder: (_, __) => const HashGeneratorPage(),
            ),
            GoRoute(
              path: 'phishing',
              builder: (_, __) => const PhishingCheckerPage(),
            ),
            GoRoute(
              path: 'binary',
              builder: (_, __) => const BinaryDecoderPage(),
            ),
            GoRoute(
              path: 'data-leak',
              builder: (_, __) => const DataLeakScannerPage(),
            ),
          ],
        ),
    ...
    GoRoute(
      path: AppConstants.routeReportHistory,
    ...
    GoRoute(
      path: AppConstants.routeCryptoChallenge,
      builder: (_, state) =>
          CryptoChallenege(challenge: state.extra as CtfChallengeModel?),
    ),
    GoRoute(
      path: AppConstants.routeForensicChallenge,
      builder: (_, state) =>
          ForensicChallenge(challenge: state.extra as CtfChallengeModel?),
    ),
    GoRoute(
      path: AppConstants.routeWebChallenge,
      builder: (_, state) =>
          WebChallenge(challenge: state.extra as CtfChallengeModel?),
    ),
    GoRoute(
      path: AppConstants.routeReverseChallenge,
      builder: (_, state) =>
          ReverseChallenge(challenge: state.extra as CtfChallengeModel?),
    ),
  ],
);
